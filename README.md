...
| 🧭 **跨会话正确搬运** | 复制时按**源存档记录的编号快照**把方块解释成方块名，粘贴时再按当前注册表落号——**编号转换只发生在粘贴那一刻，且只作用于粘贴出来的区块** |
...
2. 把 `chunkops-<版本>-mc1.12.2.jar` 放进 `mods/` 目录；
...
### 1️⃣ 改动 mods 目录会影响旧存档在游戏里的显示（但不影响本模组的搬运）

在 **REID / JEID 环境**下，部分模组（如 Forestry、EnderIO、Quark 一类）的**方块注册编号会随 mods 目录内容变化而重排**。
而 1.12.2 的存档里方块只有数字、没有名字，**游戏自己是按当前会话的注册表去读这些数字的** ——
所以如果某个模组的编号真的移动了，**游戏进旧存档时会把那些方块显示成别的方块（或空气）**，这是游戏本体的行为，本模组改不了。

**但这不影响本模组的搬运** ✅：复制时用**源存档记录的编号快照**把数字解释成方块名，粘贴时再按当前注册表落号，
**编号转换只发生在粘贴那一刻，且只作用于粘贴出来的那些区块**。所以：

- 增删模组之后，**复制粘贴（含跨存档、跨会话创建的存档）依然正确**，不需要修复整个存档；
- 同一个会话下创建的存档之间，怎么增删模组都不会串块（实测）；
- 新模组若注册在注册表末尾，已有方块的编号**根本不会移动** ⇒ 旧存档在游戏里也完全正常；
- ⚠️ 唯一例外：**本模组装上之前创建的老存档没有编号记录**，从它们复制时会给出警告（历史编号无法反推）。

**可选的稳妥习惯**：模组大改（换整合包、批量换 jar）之前先备份存档，万一游戏里显示异常还能回退。

**自检方法**：启动后看一眼 `logs/latest.log` 里的 `[ChunkOps-diag]` 行——其中的 `storageStateId` 是本次会话的编号代际，
和上次相同就说明编号没变（旧存档在游戏里也会正常显示）。
...
**Q：界面显示成 `chunkops.xxx` 这样的键名（没翻译）？**
说明资源包管线没读到模组 jar 里的语言文件。新版已加**自检 + 自载入回退**：启动后主菜单初始化时会往 `logs/latest.log` 打一行
`[ChunkOps-i18n] lang=… domains=… resZh=… resEn=… vanillaI18nOk=… chunkopsI18nOk=… fallbackUsed=…`；
即使资源包路径失效，模组也会自己从 jar 里读语言文件兜底，界面仍正常显示。若依然异常，请把这一行发到 Issue。
