...
| 🧭 **编号漂移修复** | 每次启动留档注册表快照，并给每个存档记录「它是哪套编号下写的」；mods 变动导致编号重排后，可一键把存档里的编号**对齐回当前会话**（自动备份 + 回读校验），不再只能干等重放 |
...
**万一还是变了（已内置补救）**：编辑器打开存档时如果发现「这个存档不是当前这套编号写的」，顶部会出现红色 **⚠ 编号已漂移** 标记，
点右上角 **「修复编号」** → 扫描（只读，告诉你影响多少区块）→ 再点一次确认写入，即可把存档里的编号**对齐回当前会话**，方块就恢复正常了。
原理：每次启动都会把注册表快照留档到 `chunkops/snapshots/`，每个存档也会记录"它是哪套编号下写的"，修复时按「方块名 + meta」重新对齐。
> ⚠️ 前提是**当时那份快照还在**：本功能是**从现在开始**保护你的存档；在此之前就已经写坏、且当时没有快照的数据无法反推（没有旧编号→名字的对照表）。
> 另外，mods 目录里**已被卸载的模组**的方块无法映射，会被保留原值并在报告里列出。
