...
public class ChunkOpsConfigScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        int cx = this.width / 2;
        this.buttonList.add(new GuiButton(BTN_OPEN, cx - 100, this.height / 2 - 24, 200, 20, "打开区块编辑器"));
        this.buttonList.add(new GuiButton(BTN_DONE, cx - 100, this.height / 2 + 22, 200, 20, "完成"));
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        int cx = this.width / 2;
        this.drawCenteredString(this.fontRenderer, "ChunkOps 区块编辑器", cx, this.height / 2 - 72, 0xFFFFFF);
        this.drawCenteredString(this.fontRenderer,
                "入口说明：主菜单按钮可能被主菜单美化模组（FancyMenu 等）隐藏，", cx, this.height / 2 - 54, 0xFFAAAAAA);
        this.drawCenteredString(this.fontRenderer,
                "从「模组列表 → ChunkOps → 配置」进入始终可用。", cx, this.height / 2 - 42, 0xFFAAAAAA);
